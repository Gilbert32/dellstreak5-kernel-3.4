...
static int i2c_ecompass_suspend(struct device *dev);
static int i2c_ecompass_resume(struct device *dev);
...
static const struct dev_pm_ops ecompass_akm_ak8973s_pm_ops = {
...
static struct i2c_driver i2c_ecompass_driver = {
	.driver = {
		.owner = THIS_MODULE,
		.name = ECOMPASS_NAME,
		.pm = (&ecompass_akm_ak8973s_pm_ops),
	},
