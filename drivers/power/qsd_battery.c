...
static int qsd_bat_i2c_suspend(struct device *dev)
{
...
static int qsd_bat_i2c_resume(struct device *dev)
{
...
static const struct dev_pm_ops qsd_bat_dev_pm_ops = {
...
static int __devinit qsd_bat_probe(struct platform_device *plat_dev)
{
  ...
  #if 0  
  ret = irq_set_irq_wake(qb_data.irq_bat_low, 1);
  if(ret < 0) {fail = 6;  goto err_exit;}
  #endif
  ret = irq_set_irq_wake(qb_data.irq_chg_int, 1);
