...
DEFINE_SEMAPHORE(sem_active_sensors);
DEFINE_SEMAPHORE(sem_ecompass);
DEFINE_SEMAPHORE(sem_gsensor);
DEFINE_SEMAPHORE(sem_wakeup);
...
static int report_gsensor(void)
{
	...
	sensors_daemon_data.report_count = sensors_daemon_data.report_count + 1;
	SENSORS_DAEMON_PRINTKD(KERN_DEBUG "[SENSORS DAEMON] %s-ret=%d\n", __func__, ret);
	return ret;
}
...
static long misc_sensors_daemon_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		case SENSORS_DAEMON_IOC_SET_HAL_STATUS:
			SENSORS_DAEMON_PRINTKD(KERN_DEBUG "[SENSORS DAEMON] SENSORS_DAEMON_IOC_SET_HAL_STATUS\n");
			if (copy_from_user(&hal_state, (void __user*) arg, _IOC_SIZE(cmd)))
			{
				SENSORS_DAEMON_PRINTKE(KERN_ERR "[SENSORS DAEMON] %s::SENSORS_DAEMON_IOC_SET_HAL_STATUS:copy_from_user fail-\n", __func__);
				ret = -EFAULT;
			}
			sensors_daemon_data.hal_stat = hal_state.hal_status;
			sensors_daemon_data.hal_count = hal_state.hal_count;
			break;
...
{
	return snprintf(buf, PAGE_SIZE, "%d\n", sensors_daemon_data.hal_stat);
}

static ssize_t
show_count(struct device *dev, struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", sensors_daemon_data.hal_count);
}

static ssize_t
show_report_count(struct device *dev, struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", sensors_daemon_data.report_count);
}

static ssize_t
show_accel(struct device *dev, struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "x:%d, y:%d, z:%d\n", sensors_daemon_data.accel_x, sensors_daemon_data.accel_y, sensors_daemon_data.accel_z);
}


static DEVICE_ATTR(status, 0444,
		show_status, NULL);
static DEVICE_ATTR(count, 0444,
		show_count, NULL);
static DEVICE_ATTR(report_count, 0444,
		show_report_count, NULL);
static DEVICE_ATTR(accel, 0444,
		show_accel, NULL);
...
static int __init sensors_daemon_init(void)
{
	...
	ret = sysfs_create_group(&(misc_sensors_daemon.this_device)->kobj, &dev_attr_grp);
