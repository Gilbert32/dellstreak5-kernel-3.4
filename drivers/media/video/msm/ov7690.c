...
static int32_t ov7690_i2c_read( struct ov7690_reg_t *regs, uint32_t size)
{
    ...
    {
        CDBG("ov7690_i2c_read: invalid client\n");
        return -EFAULT;
    }
    ...
    for( i=0; i<size; i++ )
    {
        ...
        if ( ARRAY_SIZE(msg) != i2c_transfer( ov7690_client->adapter, msg, 2) ) {
            CDBG("ov7690_i2c_read: read %Xh failed\n", buf[0] );
            return -EIO;
        }
#else
        for( retryCnt=0; retryCnt<MAX_RETRY_COUNT; retryCnt++)
        {
            ...
            {
                if( retryCnt > 0 )
                    CDBG("ov7690_i2c_read: read %Xh=0x%X successfull at retryCnt=%d\n",
...
static int32_t ov7690_i2c_write( struct ov7690_reg_t *regs, uint32_t size)
{
    ...
    if( isValidClient(ov7690_client) )
    {
        CDBG("ov7690_i2c_write: invalid client\n");
        return -EFAULT;
    }
    ...
    for( i=0; i<size; i++ )
    {
        if( OV7690_BIT_OPERATION_TAG == regs->addr )
        {
            ...
            if( !ov7690_i2c_read( &writableReg, 1 ) )
            {
                ...
                CDBG("ov7690_i2c_write: read %Xh before write failed\n", writableReg.addr);
                return -EIO;
            }
        ...
        if ( ARRAY_SIZE(msg) != i2c_transfer( ov7690_client->adapter, msg, 1) ) {
            CDBG("ov7690_i2c_write: write %Xh=0x%X failed\n", buf[0], buf[1] );
            return -EIO;
        }
#else
        for( retryCnt=0; retryCnt<MAX_RETRY_COUNT; retryCnt++)
        {
            ...
            {
                if( retryCnt > 0 )
                    CDBG("ov7690_i2c_write: write %Xh=0x%X successfull at retryCnt=%d\n",
...
static int32_t ov7690_i2c_burst_read( unsigned short addr, unsigned char *data, uint32_t size)
{
    ...
    if( isValidClient(ov7690_client) )
    {
        CDBG("ov7690_i2c_burst_read: invalid client\n");
        return -EFAULT;
    }

    if ( i2c_transfer(ov7690_client->adapter, msg, ARRAY_SIZE(msg)) != ARRAY_SIZE(msg) ) {
        CDBG( "ov7690_i2c_burst_read: read %Xh %d bytes failed\n", addr, size );
        return -EIO;
    }
...
static long ov7690_engineer_mode( int type, void *pCfg )
{
    ...
    switch(type)
    {
        case CFG_DUMP_SINGLE_REGISTER:
        {
            ...
            if( !rc )
            {
                CDBG("ov7690_engineer_mode: read %Xh=0x%X\n", reg.addr, reg.data);
        ...
        case CFG_DUMP_REGISTERS_TO_FILE:
        {
            ...
            {
                rc = -EFAULT;
                CDBG("ov7690_engineer_mode: invalid args\n");
                break;
            }
        ...
        case CFG_SET_REGISTERS_FROM_FILE:
        {
            ...
            for(i=0; i < pOrigCfg->len; i+=2)
            {
                ...
                if( OV7690_BIT_OPERATION_TAG == reg.addr )
                {
                    ...
                    if( i+2 > pOrigCfg->len )
                    {
                        CDBG("ov7690_engineer_mode: bit operation is broken\n");
                        rc = -1;
                        break;
                    }
                    ...
                    CDBG("ov7690_engineer_mode: read  %Xh=%X\n", reg.addr, reg.data);

                    reg.data &= ~mask;
                    reg.data |= buf[i+1];
                }

                CDBG("ov7690_engineer_mode: write %Xh=%X\n", reg.addr, reg.data);
                rc = ov7690_i2c_write( &reg, 1 );
                if( rc )
                    break;
            }
        ...
        default:
            CDBG("ov7690_engineer_mode: not supported type\n");
            rc = -EFAULT;
            break;
    }
                                   ...
                                   int res)
{
    ...
    switch( mode )
    {
        ...
        case SENSOR_SNAPSHOT_MODE:
            ...
            break;
        default:
            CDBG( "ov7690_set_sensor_mode: unknown mode %d\n", mode );
            rc = -1;
            break;
    }
...
static long ov7690_set_wb( int8_t whiteBalance )
{
    ...
    switch((enum camera_wb_t)whiteBalance) {
        ...
        break;
        default: {
            CDBG("ov7690_set_wb: unsupported wb %d\n", whiteBalance);
            rc = -EFAULT;
        }
        break;
    }
...
static long ov7690_set_brightness( int8_t brightness )
{
    ...
    switch(brightness) {
        ...
        break;
        default: {
            CDBG("ov7690_set_brightness: unsupported brightness %d\n", brightness);
            rc = -EFAULT;
        }
        break;
    }
...
static long ov7690_set_contrast( int8_t contrast )
{
    ...
    switch(contrast) {
        ...
        break;
        default: {
            CDBG("ov7690_set_contrast: unsupported contrast %d\n", contrast);
            rc = -EFAULT;
        }
        break;
    }
...
static long ov7690_set_antibanding( int16_t banding )
{
    long rc = 0;
    ...
    switch((enum camera_antibanding_t)banding) {
        ...
        break;
        default: {
            CDBG("ov7690_set_antibanding: unsupported banding %d\n", banding);
            rc = -EFAULT;
        }
        break;
    }
...
static long ov7690_set_scene( int8_t scene )
{
    ...
    switch(scene) {
        ...
        break;
        default: {
            CDBG("ov7690_set_scene: unsupported scene %d\n", scene);
            rc = -EFAULT;
        }
        break;
    }
...
static int32_t ov7690_sensor_detect( void )
{
    ...
    if( !rc )
        ...
        {
            CDBG( "ov7690_sensor_detect: id 0x%X 0x%X not matched\n",
...
static int ov7690_sensor_init_probe(const struct msm_camera_sensor_info *data)
{
    ...
    if( rc < 0 )
    {
        CDBG("ov7690_sensor_init: sensor detect failed\n");
        goto exit_init_probe;
    }
...
int ov7690_sensor_preinit(void *info)
{
    ...
    {
        rc = -1;
        CDBG("ov7690_sensor_preinit: invalid arg\n");
        goto exit_preinit;
    }
    ...
    rc = ov7690_reg_init();
    if( rc < 0 ) {
        CDBG("ov7690_sensor_preinit(): reg init failed\n");
        goto reg_init_fail;
    }
    ...
    if( ov7690_client )
    {
        ...
        if( rc < 0 )
            CDBG("ov7690_sensor_init: output disabled failure\n");
    }
...
init_fail:
    kfree(ov7690_ctrl);
    ov7690_ctrl = NULL;
    CDBG("ov7690_sensor_init(): init failed\n");
...
int ov7690_sensor_config(void __user *argp)
{
        ...
        case CFG_SET_REGISTERS_FROM_FILE:
            ...
            if( !rc && CFG_DUMP_SINGLE_REGISTER == cfg_data.cfgtype )
            {
                if ( copy_to_user( (void *)argp,
                                    ...
                                    sizeof(struct sensor_cfg_data)) )
                {
                    CDBG("ov7690_ioctl: copy to user failed\n");
                    rc = -EFAULT;
                }
            }
        break;
    ...
    if( rc ) CDBG("ov7690_ioctl(), type=%d, mode=%d, rc=%d\n", cfg_data.cfgtype, cfg_data.mode, rc);
