...
int32_t ov5642_i2c_write( struct ov5642_reg_t *regs, uint32_t size)
{
    ...
    if( isValidClient(ov5642_client) )
    {
        CDBG("ov5642_i2c_write: invalid client\n");
        return -EFAULT;
    }
    ...
    for( i=0; i<size; i++ )
    {
        if( OV5642_BIT_OPERATION_TAG == regs->addr )
        {
            ...
            if( !ov5642_i2c_read( &writableReg, 1 ) )
            {
                ...
                CDBG("ov5642_i2c_write: read %Xh before write failed\n", writableReg.addr);
                return -EIO;
            }
        ...
        if ( ARRAY_SIZE(msg) != i2c_transfer( ov5642_client->adapter, msg, 1) ) {
            CDBG("ov5642_i2c_write: write %Xh=0x%X failed\n", (buf[0]<<8)|buf[1], buf[2] );
            return -EIO;
        }
#else
        for( retryCnt=0; retryCnt<MAX_RETRY_COUNT; retryCnt++)
        {
            ...
            {
                if( retryCnt > 0 )
                    CDBG("ov5642_i2c_write: write %Xh=0x%X successfull at retryCnt=%d\n",
...
int32_t ov5642_i2c_burst_read( unsigned short addr, unsigned char *data, uint32_t size)
{
    ...
    if( isValidClient(ov5642_client) )
    {
        CDBG("ov5642_i2c_burst_read: invalid client\n");
        return -EFAULT;
    }
    ...
    if ( i2c_transfer(ov5642_client->adapter, msg, ARRAY_SIZE(msg)) != ARRAY_SIZE(msg) ) {
        CDBG( "ov5642_i2c_burst_read: read %Xh %d bytes failed\n", addr, size );
        return -EIO;
    }
#else
    for( retryCnt=0; retryCnt<MAX_RETRY_COUNT; retryCnt++)
    {
        ...
        {
            if( retryCnt > 0 )
                CDBG("ov5642_i2c_burst_read: read %Xh %d bytes successfully at retryCnt=%d\n",
                           addr, size, retryCnt);
            break;
        }
    }
...
static long ov5642_engineer_mode( int type, void *pCfg )
{
    ...
    switch(type)
    {
        case CFG_DUMP_SINGLE_REGISTER:
        {
            ...
            if( !rc )
            {
                CDBG("ov5642_engineer_mode: read %Xh=0x%X\n", reg.addr, reg.data);
        ...
        case CFG_DUMP_REGISTERS_TO_FILE:
        {
            ...
            {
                rc = -EFAULT;
                CDBG("ov5642_engineer_mode: invalid args\n");
                break;
            }
        ...
        case CFG_SET_REGISTERS_FROM_FILE:
        {
            ...
            for(i=0; i < pOrigCfg->len; i+=3)
            {
                ...
                if( OV5642_BIT_OPERATION_TAG == reg.addr )
                {
                    ...
                    if( i+3 > pOrigCfg->len )
                    {
                        CDBG("ov5642_engineer_mode: bit operation is broken\n");
                        rc = -1;
                        break;
                    }
                    ...
                    CDBG("ov5642_engineer_mode: read  %Xh=%X\n", reg.addr, reg.data);

                    reg.data &= ~mask;
                    reg.data |= buf[i+2];
                }

                CDBG("ov5642_engineer_mode: write %Xh=%X\n", reg.addr, reg.data);
                rc = ov5642_i2c_write( &reg, 1 );
                if( rc )
                    break;
        ...
        default:
            CDBG("ov5642_engineer_mode: not supported type\n");
            rc = -EFAULT;
            break;
    }
...
struct timespec temp;
    ...
    switch( mode )
    {
        ...
        default:
        {
            CDBG( "ov5642_set_sensor_mode: unknown mode %d\n", mode );
            rc = -1;
            break;
        }
    }
...
static long ov5642_set_brightness( int8_t brightness )
{
    ...
    switch(brightness) {
        ...
        break;
        default: {
            CDBG("ov5642_set_brightness: unsupported brightness %d\n", brightness);
            rc = -EFAULT;
        }
        break;
    }
...
static long ov5642_set_contrast( int8_t contrast )
{
    ...
    switch(contrast) {
        ...
        break;
        default: {
            CDBG("ov5642_set_contrast: unsupported contrast %d\n", contrast);
            rc = -EFAULT;
        }
        break;
    }
...
static long ov5642_set_antibanding( int16_t banding )
{
    long rc = 0;
    ...
    switch((enum camera_antibanding_t)banding) {
        ...
        break;
        default: {
            CDBG("ov5642_set_antibanding: unsupported banding %d\n", banding);
            rc = -EFAULT;
        }
        break;
    }
...
static int32_t ov5642_sensor_detect( void )
{
    ...
    if( !rc )
        ...
        {
            CDBG( "ov5642_sensor_detect: id 0x%X 0x%X not matched\n",
...
static int ov5642_sensor_init_probe(const struct msm_camera_sensor_info *data)
{
    ...
    if( rc < 0 )
    {
        CDBG("ov5642_sensor_init: sensor detect failed\n");
        goto exit_init_probe;
    }
...
int ov5642_sensor_preinit(void *info)
{
    ...
    {
        rc = -1;
        CDBG("ov5642_sensor_preinit: invalid arg\n");
        goto exit_preinit;
    }

    ov5642_ctrl = kzalloc(sizeof(struct ov5642_ctrl), GFP_KERNEL);
    if (!ov5642_ctrl) {
        CDBG("ov5642_sensor_preinit failed!\n");
        rc = -ENOMEM;
        goto exit_preinit;
    }
    ...
    rc = ov5642_reg_init();
    if( rc < 0 ) {
        CDBG("ov5642_sensor_preinit(): reg init failed\n");
        goto reg_init_fail;
    }
    ...
    if( ov5642_client )
    {
        ...
        if( rc < 0 )
            CDBG("ov5642_sensor_init: output disabled failure\n");
    }
...
int ov5642_sensor_init(const struct msm_camera_sensor_info *data)
{
    ...
    if( system_rev < EVT2_Band125 )
    {
        ov5642_ctrl->reg_map = &ov5642_1C_regs;
        ov5642_ctrl->ae_func = &ov5642_1C_ae_func;
        CDBG("ov5642_sensor_init: 1C sensor\n");
    } else {
        ov5642_ctrl->reg_map = &ov5642_1D_regs;
        ov5642_ctrl->ae_func = &ov5642_1D_ae_func;
        CDBG("ov5642_sensor_init: 1D sensor\n");
    }
...
init_fail:
    kfree(ov5642_ctrl);
    ov5642_ctrl = NULL;
    CDBG("ov5642_sensor_init(): init failed\n");
...
int ov5642_sensor_config(void __user *argp)
{
    ...
    if( rc ) CDBG("ov5642_ioctl(), type=%d, mode=%d, rc=%d\n", cfg_data.cfgtype, cfg_data.mode, rc);
