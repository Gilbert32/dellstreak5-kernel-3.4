...
static int misc_lsensor_open(struct inode *inode_p, struct file *fp)
{
  ...
  MSG("%s    <== [%d]",__func__,ls_data.opened);
  return 0;
}
static int misc_lsensor_release(struct inode *inode_p, struct file *fp)
{
  MSG("%s <== [%d]\n",__func__,ls_data.opened);
  ls_data.opened --;
  return 0;
}

static long misc_lsensor_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
...
static struct file_operations misc_lsensor_fops = {
  ...
  .unlocked_ioctl  = misc_lsensor_ioctl,
};
