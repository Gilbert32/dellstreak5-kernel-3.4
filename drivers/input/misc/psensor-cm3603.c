...
static long misc_psensor_ioctl(struct file *fp, unsigned int cmd, unsigned long arg);
...
static struct file_operations misc_psensor_fops = {
	...
	.compat_ioctl = misc_psensor_ioctl,
	.read = NULL,
	.write = NULL,
};
	...
	switch (cmd)
	{
		case PSENSOR_IOC_ENABLE:
			...
			if(psensor_power_counter == 1)
			{
				...
				if(distance)
				 irq_set_irq_type(MSM_GPIO_TO_INT(PSENSOR_INT_GPIO), IRQF_TRIGGER_FALLING);
				else
				 irq_set_irq_type(MSM_GPIO_TO_INT(PSENSOR_INT_GPIO), IRQF_TRIGGER_RISING);
...
static void psensor_work_func(struct work_struct *work)
{
	...
	if(ps_out_status != last_ps_out_status)
	{
		if(ps_out_status == 0)
		{
			...
			{
				if (voice_started)  atomic_set(&psensor_approached, 1);
				irq_set_irq_type(MSM_GPIO_TO_INT(PSENSOR_INT_GPIO), IRQF_TRIGGER_RISING);
			        last_ps_out_status = ps_out_status;
