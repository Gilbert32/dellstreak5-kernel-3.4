...
static long misc_cover_ioctl(struct file *fp, unsigned int cmd, unsigned long arg);
static int reset_chip(void);
void final_sync_files(void);
extern unsigned int system_rev;

static struct file_operations cover_fops = {
	.unlocked_ioctl = misc_cover_ioctl
};
...
static void cover_work_func(struct work_struct *work)
{
	...
	else
	{
        ...
        final_sync_files();
        #endif
		printk(KERN_WARNING "Cover removed.\n");
		kernel_power_off();
	}
}
...
int cover_irq_setup(int cover_int_pin)
{
    ...
    {
        irq_set_irq_wake(MSM_GPIO_TO_INT(cover_int_pin), 1);    
    }             
    return rc;
}
