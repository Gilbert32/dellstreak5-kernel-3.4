...
#endif
DEFINE_SEMAPHORE( ts_sem );

static uint8_t             g_touch_suspended = 0;
DEFINE_SEMAPHORE( ts_suspend_sem );
...
//n0p - we need to kick cpu here.
                printk ("DSC: kick CPU on touchscreen resume");
                acpuclk_set_rate(0, 512000, SETRATE_CPUFREQ);
		usleep(200);
                printk("DSC Voodoo: TS resume");
		rc = ts_resume_ic(g_ts, client);
                acpuclk_set_rate(0, 768000, SETRATE_CPUFREQ);
		usleep(100);
...
static int ts_remove(struct i2c_client *client)
{
    ...
    cancel_delayed_work_sync(&ts->touch_work);
