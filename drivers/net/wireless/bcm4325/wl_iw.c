 ...
 *
 * $Id: wl_iw.c,v 1.51.4.9.2.8.6.62 2009/11/25 22:04:25 Exp $
 */
...
#define WL_IW_POWER_OFF_THREAD 1
...
)
{
	...
	if (strnicmp((char *)&powermode_val, "1", strlen("1")) == 0) {
		...
		if (g_bt->timer_on) { // Just in case the timer already started.
			printk(KERN_INFO "%s: Error! Timer already started. Delete it first.\n",__func__);
...
)
{
	 wl_iw_t *iw = *(wl_iw_t **)netdev_priv(dev);
        ...
        WAKE_LOCK_INIT(iw->pub, WAKE_LOCK_OFF, "sysioc_wl_off");
...
)
{
    ...
    if(dev_wlc_bufvar_set(dev,"arp_table_clear",NULL,0)<0)
        printk("### bear unable to clean arp table\n");
	...
	for (i = 0; i < ss_list->count; i++) {
		...
		for (;node;) {
			if (!memcmp(&node->bss_info->BSSID, &bi->BSSID, ETHER_ADDR_LEN) && 
                            !memcmp(&node->bss_info->ie_length, &bi->ie_length, sizeof(uint32))) {
...
)
{
	...
	if (dwrq->length && extra) {
		...
		temp.SSID_len = g_ssid.SSID_len;
		memcpy(temp.SSID, extra, temp.SSID_len);
	} else {
	...
	printk("[WH Lee] WLC_SET_SSID[2]\n");
	if ((error = dev_wlc_ioctl(dev, WLC_SET_SSID, &temp, sizeof(temp))))
		return error;
...
)
{
	...
	memset(country_code, 0, sizeof(country_offset));
	country_offset = strcspn(extra, " ");
	country_code_size = strlen(extra) - country_offset;

        if (country_code_size == 0)
        {
                if((error = dev_wlc_ioctl(dev, WLC_GET_COUNTRY,
                       &country_code, sizeof(country_code))) >= 0)
                {
		    if(strncmp(country_code,"US",2)==0)
                         p += snprintf(p, MAX_WX_STRING, "Scan-Channels = 11");
		    else if(strncmp(country_code,"EU",2)==0)
                         p += snprintf(p, MAX_WX_STRING, "Scan-Channels = 13");
		    else if(strncmp(country_code,"JP",2)==0)
                         p += snprintf(p, MAX_WX_STRING, "Scan-Channels = 14");
                    else
                         p += snprintf(p, MAX_WX_STRING, "Scan-Channels = 11");
                    goto exit;
                }
	...
	if (dwrq->length && extra) {
	    ...
	    if (strnicmp(extra, "SCAN-ACTIVE", strlen("SCAN-ACTIVE")) == 0) {
			...
			ret = wl_iw_get_macaddr(dev, info, (union iwreq_data *)dwrq, extra);
		        ...
		        ret = wl_iw_get_set_country(dev, info, (union iwreq_data *)dwrq, extra);
...
#ifdef SOFTAP
	    else if (strnicmp(extra, "Q_STOP", strlen("Q_STOP")) == 0)
			ret = wl_iw_q_stop(dev, info);
...
static const iw_handler wl_iw_priv_handler[] = {
	...
	(iw_handler)wl_iw_get_set_country,
